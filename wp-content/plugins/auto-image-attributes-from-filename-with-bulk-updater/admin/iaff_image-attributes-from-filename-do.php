...
function iaff_clean_filename( $file ) {
	...
	if ( isset( $settings['commas'] ) && boolval($settings['commas']) ) {
		...
		// $file['name'] = sanitize_title_with_dashes( $file['name'] );
...
function iaff_update_image( $image_id, $text, $bulk = false ) {
	...
	if ( $bulk == true ) {
		...
		$image['post_title'] 	= $image_name = ucwords( $text );	// Image Title
		...
		update_post_meta( $image_id, '_wp_attachment_image_alt', ucwords( $text ) ); // Image Alt Text

...
use \CODEQR\Response as Response;
...
class Autoload {
	...
	public function apiLoaded() {
		...
		if( empty( $wp->query_vars[QRROUTE] ) )
			
			return;

		spl_autoload_register( array($this, 'autoloader') );

		$new_class = '\\CODEQR\\' . ucfirst( strtolower( qrgetController() ) ) . 'Controller';

		$class = new $new_class();
		
		$action_name = str_replace('-', '_', qrgetMethod()) . $_SERVER['REQUEST_METHOD'];
	...
	public function autoloader( $class_name ) {
		...
		if( strpos( $class_name, 'CODEQR' ) !== false ) {
			
			$api_path = dirname( __DIR__ ) . '/source/' . VERSION;
			
			if( strpos( $class_name, 'Controller' ) !== false ) {
				
				$target_path = $api_path . '/controllers/' . str_replace( 'CODEQR\\', '', $class_name ) . '.php';
			...
			elseif( strpos( $class_name, 'Model' ) !== false ) {
				...
				$subpath = str_replace( 'CODEQR\\', '', $class_name );

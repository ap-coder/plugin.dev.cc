...
class Autoload {
	...
	public function apiLoaded() {
		...
		$new_class = '\\WPMVC\\' . ucfirst( strtolower( getController() ) ) . 'Controller';
	...
	public function autoloader( $class_name ) {
		...
		if( strpos( $class_name, 'WPMVC' ) !== false ) {
			
			$api_path = dirname( __DIR__ ) . '/source/' . VERSION;
			
			if( strpos( $class_name, 'Controller' ) !== false ) {
				
				$target_path = $api_path . '/controllers/' . str_replace( 'WPMVC\\', '', $class_name ) . '.php';
			...
			elseif( strpos( $class_name, 'Model' ) !== false ) {
				...
				$subpath = str_replace( 'WPMVC\\', '', $class_name );

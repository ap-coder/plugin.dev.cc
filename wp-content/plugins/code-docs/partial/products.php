		...
		<thead>
			<tr>
				...
				<th class="manage-column column-columnname sortable <?php echo get_sort_classes($slug); ?>" scope="col" data-state="false" data-col="<?php echo $column; ?>">
					<a href="<?php echo get_admin_url();?>admin.php?page=codedocs-products&<?php echo fill_query_string(array('orderby'=>$slug, 'order'=>$direction))?>"><span><?php echo $column; ?></span><span class="sorting-indicator"></span></a>
				 </th>
				<?php endforeach;?>
		...
		<tbody>
			<?php foreach($results as $key=>$result):?>
				<tr <?php echo $key % 2 == 0 ? 'class="alternate"':''; ?> data-row="<?php echo $result->id;?>">
					...
					<td>[docset product_id="<?php echo $result->ID;?>"]</td>
					<td class="column-columnname">
	          <a style="color: #00a0d2;" href="<?php echo site_url( '/wp-admin/admin.php?page=codedocs-products&product_id='.$result->ID, $scheme = null )?>">View</a>
					</td>
				</tr>
			<?php endforeach;?>
		</tbody>

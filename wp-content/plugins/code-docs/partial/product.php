...
?>
<div class="wrap">
	<h2><?php esc_html_e( 'Manage Product Documentation' ); ?></h2>
	<hr>

	<form method="POST" action="<?=get_admin_url();?>admin.php?page=codedocs-products&product_id=<?php echo $_GET['product_id']?>">
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td valign="top" style="vertical-align: top; width: 50%; background-color: #f9f9f9; box-shadow: 0px 0px 3px 0px inset #ddd;">
							<h5>Available Documents</h5>
							<ul id="product-list" style="max-height: 600px; min-height: 300px; overflow-y: scroll; vertical-align:top; padding-right: 10px;">
								<?php 
									global $wpdb;
						...
						<td  style="width: 50%; background-color: #f9f9f9; box-shadow: 0px 0px 3px 0px inset #ddd;vertical-align:top">
							<?php $row = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}sort WHERE `sort_type`='product' AND `type_id`= ".$_GET['product_id']); 
							...
							<?php
									...
									$documents = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}files WHERE parent_id IS NULL AND FIND_IN_SET('{$_GET['product_id']}',product)");
							?>
	...
	jQuery(document).ready(function($){
		...
		$('.btn.btn-select').on('click', function(){
			...
			$('#selected-products').append('<li>\
										<div class="left" style="padding-right: 5px;"><span><i class="fa fa-bars"></i></span></div><div class="none"><span class="btn btn-unselect"><i class="fa fa-minus"></i></span>'+label+'<input type="hidden" name="documents[]" value="'+doc_id+'" /></div></li>')
			$(this).closest('li').hide();
		});

		$('#selected-products').on('click', '.btn-unselect', function(){
			var doc_id = $(this).closest('div.none').find('input').val();
			$('#product-list li[data-doc_id='+doc_id+']').show();

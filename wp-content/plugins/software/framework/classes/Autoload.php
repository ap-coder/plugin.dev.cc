...
use \SOFTWARE\Response as Response;
...
class Autoload {
	...
	public function apiLoaded() {
		...
		if( empty( $wp->query_vars[SWROUTE] ) )
		...
		$new_class = '\\SOFTWARE\\' . ucfirst( strtolower( swgetController() ) ) . 'Controller';

		$class = new $new_class();
		
		$action_name = str_replace('-', '_', swgetMethod()) . $_SERVER['REQUEST_METHOD'];
	...
	public function autoloader( $class_name ) {
		...
		if( strpos( $class_name, 'SOFTWARE' ) !== false ) {
			
			$api_path = dirname( __DIR__ ) . '/source/' . SWVERSION;
			
			if( strpos( $class_name, 'Controller' ) !== false ) {
				
				$target_path = $api_path . '/controllers/' . str_replace( 'SOFTWARE\\', '', $class_name ) . '.php';
			...
			elseif( strpos( $class_name, 'Model' ) !== false ) {
				...
				$subpath = str_replace( 'SOFTWARE\\', '', $class_name );

...
if( !function_exists('swroute') ){
  function swroute($path){
    return site_url() . '/software/' .ltrim($path, '/');
  }
}
...
if( !function_exists('swgetController') ){
  function swgetController(){

    global $wp;

    if(isset($wp->query_vars[SWROUTE])){

      $path = explode( '/', trim( $wp->query_vars[SWROUTE], '/' ) );
...
if( !function_exists('swgetMethod') ){
  function swgetMethod(){
...
if( !function_exists('swparseUrl') ){
  function swparseUrl(){

    global $wp;

    $url = explode( '/', trim( $wp->query_vars[SWROUTE], '/' ) );

    unset($url[array_search(swgetController(), $url)]);

    foreach($url as $key=>$param)
      if($param == swgetMethod())
        unset($url[$key]);
